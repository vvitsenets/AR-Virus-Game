...
class ViewController: UIViewController, ARSCNViewDelegate, SCNPhysicsContactDelegate {
    ...
    //paper button
    @IBAction func onPaperButton(_ sender: Any) {
        fireMissile(type: "paper")
    }
    
    //mask button
    @IBAction func onMaskButton(_ sender: Any) {
        fireMissile(type: "mask")
    }
    ...
    //creates mask or paper node and 'fires' it
    ...
    func createMissile(type : String)->SCNNode{
        ...
        switch type {
        case "mask":
            let scene = SCNScene(named: "art.scnassets/mask.dae")
            node = (scene?.rootNode.childNode(withName: "mask", recursively: true)!)!
            node.scale = SCNVector3(0.2,0.2,0.2)
            node.name = "mask"
        case "paper":
            let scene = SCNScene(named: "art.scnassets/paper.dae")
            node = (scene?.rootNode.childNode(withName: "paper", recursively: true)!)!
    ...
    func addTargetNodes(){
        for index in 1...100 {
            ...
            if (index > 9) && (index % 10 == 0) {
                let scene = SCNScene(named: "art.scnassets/virus2.dae")
                node = (scene?.rootNode.childNode(withName: "virus2", recursively: true)!)!
                node.scale = SCNVector3(0.3,0.3,0.3)
                node.name = "virus2"
            }else{
                let scene = SCNScene(named: "art.scnassets/virus4.dae")
                node = (scene?.rootNode.childNode(withName: "virus4", recursively: true)!)!
                node.scale = SCNVector3(0.2,0.2,0.2)
                node.name = "virus4"
            }
...
*/
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        if contact.nodeA.physicsBody?.categoryBitMask == CollisionCategory.targetCategory.rawValue
            ...
            if (contact.nodeA.name! == "virus2" || contact.nodeB.name! == "virus2") {
                score+=5
            }else{
                score+=1
            }
